...
# %%
inp_year=sys.argv[1]
data_path = f"/home/kffeng/DATA_PNW/pnw_{inp_year}/STACK_acfs_HH_{inp_year}/"
...
# %%
start = datetime(int(inp_year), 1, 1, tzinfo=timezone.utc)
end = datetime(int(inp_year)+1, 1, 1, tzinfo=timezone.utc)
timerange = DateTimeRange(start, end)

# %%
# --- print results to file
outdir="./"
fcsv=outdir+f"/OUTPUT_HH_{inp_year}.csv"
file = open(fcsv, "w")
...
fn_sta=outdir+f"/station_list_HH_{inp_year}.csv"
...
png_path=f'PNGs_robust_{inp_year}'
os.makedirs(png_path, exist_ok=True)
os.system(f'mv *.png  to {png_path}')
